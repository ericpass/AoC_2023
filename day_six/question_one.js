...
const partOne = () => {
  ...
  const times = lines[0].split(':')[1].split(' ').filter((val) => val).map(Number);
  const durations = lines[1].split(':')[1].split(' ').filter((val) => val).map(Number);
  ...
  times.forEach((t, idx) => {
    let winningNums = 0;
    for (let i = 0; i < t; i++) {
      if ((i * (t - i)) > durations[idx]) {
        winningNums++;
      }
    }
    totals.push(winningNums);
  });
